...
- [ ] Sync status + backfill; unlink — _unlink shipped in the repo-linking PR; sync status + backfill not started. The scheduler now exists, so this — and Phase 10's daily `cycle_snapshot` writer, and Phase 16's daily snapshot table — are unblocked whenever they're next in line_
...
| 12 GitHub | `feat/12-github` | 🚧 substantially done: vault→PAT, repo linking (+ picker), branch/PR creation, mergeable+reviews, dev panel (live commits/CI), rate-limit handling, webhooks (receiver, installable, replay guard, push/PR/check_suite), poll fallback for hookless repos via the new `scheduler` (#12, #15–#17, #25–#33, #42). Remaining: sync status + backfill — no longer blocked (the scheduler exists now), just not started |
