...
- [ ] Persist transcripts; resume across restarts; per-session status on card — _`domain::agent_session` persists a session's status and terminal outcome (`agent_sessions`); the full event transcript is not stored, and nothing wires this to a real run yet (see the two bullets above)_
...
| 13 Claude agent | `feat/13-claude-agent` | 🚧 the runner core, workspaces, and session persistence: `stream-json` event parsing + outcome interpretation, `AgentRunner`/`LocalRunner` spawning the CLI as its own killable process group, `agent::workspace::prepare` cloning/refreshing a project's linked repo (credential never touches argv or the stored git config), and `domain::agent_session` recording a run's status/outcome. All tested against shell-script/local-git stand-ins — no real subprocess, network call, or API cost in CI. Remaining: wiring these three together behind a "Run with Claude" endpoint (nothing calls them yet), WebSocket-driven live session UI, transcript persistence, the Atlas MCP server, and per-project permission-mode configuration |
