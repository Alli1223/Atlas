...
- [ ] Persist transcripts; resume across restarts; per-session status on card — _`domain::agent_session` persists a session's status and terminal outcome, and is now wired to real runs (see the card → task binding bullet below); the full event transcript is still not stored, and a restart still loses an in-flight run's outcome (the detached drain task dies with the process — the CLI's own child is still reaped via `KillOnDrop`, so nothing keeps spending, but the session is left `running` forever with no error recorded)_
...
| 13 Claude agent | `feat/13-claude-agent` | 🚧 the runner core, workspaces, session persistence, and the wiring endpoint: `stream-json` event parsing + outcome interpretation, `AgentRunner`/`LocalRunner` spawning the CLI as its own killable process group, `agent::workspace::WorkspacePreparer`/`GitWorkspacePreparer` cloning/refreshing a project's linked repo (credential never touches argv or the stored git config), `domain::agent_session` recording a run's status/outcome, and `agent::orchestrator::start` wiring all three behind `POST /cards/{key}/agent-sessions` ("Run with Claude": prompt = summary + description) plus `GET .../agent-sessions` and `GET /agent-sessions/{id}` to read one back. All tested against shell-script/fake-preparer stand-ins — no real subprocess, network call, or API cost in CI. Remaining: cancelling a run over the API (the plumbing exists, nothing calls it), WebSocket-driven live session UI, transcript persistence, the Atlas MCP server, on-completion actions, and per-project permission-mode configuration |
