...
- [ ] Poll fallback when no public webhook URL — _the trigger condition now exists (`ATLAS_PUBLIC_URL` unset), but the actual poll mechanism doesn't — Atlas has no background/interval task runner at all yet, so this needs one built, not just wired_
