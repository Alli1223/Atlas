...
| 13 Claude agent | `feat/13-claude-agent` | 🚧 the runner core plus workspaces: `stream-json` event parsing + `is_error`/`terminal_reason`/`permission_denials` interpretation, `AgentRunner`/`LocalRunner` spawning the CLI as its own killable process group, and `agent::workspace::prepare` cloning/refreshing a project's linked repo (credential never touches argv or the stored git config). All tested against shell-script/local-git stand-ins — no real subprocess, network call, or API cost in CI. Remaining: wiring the two together (nothing calls `prepare` yet), WebSocket-driven live session UI, transcript persistence, the Atlas MCP server, card→task binding, and per-project permission-mode configuration |
