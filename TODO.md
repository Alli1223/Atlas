...
| 13 Claude agent | `feat/13-claude-agent` | 🚧 the runner core, workspaces, session + transcript persistence, the wiring endpoint, and a basic frontend: `stream-json` event parsing + outcome interpretation, `AgentRunner`/`LocalRunner` spawning the CLI as its own killable process group, `agent::workspace::WorkspacePreparer`/`GitWorkspacePreparer` cloning/refreshing a project's linked repo (credential never touches argv or the stored git config), `domain::agent_session` recording a run's status/outcome, `domain::agent_session_transcript` recording every raw line, and `agent::orchestrator::start` wiring it all behind `POST /cards/{key}/agent-sessions` ("Run with Claude": prompt = summary + description) plus `GET .../agent-sessions`, `GET /agent-sessions/{id}`, and `GET /agent-sessions/{id}/transcript`. A card-detail panel (`AgentSessions.tsx`) makes it clickable, with a polling session-history list. All backend tested against shell-script/fake-preparer stand-ins — no real subprocess, network call, or API cost in CI (#41, #45–#46). Remaining: cancelling a run over the API (the plumbing exists, nothing calls it), resuming across a restart, WebSocket-driven live streaming, the Atlas MCP server, on-completion actions, and per-project permission-mode configuration |
